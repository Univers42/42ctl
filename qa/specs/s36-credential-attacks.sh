...
# the cryptography: they need only patience and a list of email addresses.
#
...
assert_spec "signup does not reveal whether an address is already registered" \
...
# Twenty wrong passwords in a row, then the right one. If the right one still works, there is
# no lockout and no backoff, and the only thing between an attacker and an account is how
# long they are willing to wait.
assert_spec "a run of failed logins is eventually refused rather than answered forever" \
...
assert_green "requesting a one-time code does not reveal whether the account exists" \
	-- bash -c 'a=$(qa_code POST /v1/auth/otp/request "" "{\"email\":\"$2\"}")
		...
		[ "$a" = "$b" ]' _ "$N" "$MAIL"
assert_spec "one-time codes cannot be requested without limit" \
	-- bash -c 'for i in $(seq 1 10); do
			qa_code POST /v1/auth/otp/request "" "{\"email\":\"$1\"}" >/dev/null
		done
		last=$(qa_code POST /v1/auth/otp/request "" "{\"email\":\"$1\"}")
		[ "$last" = 429 ]' _ "$MAIL"
