...
ORG="org27-$N"
PUUID="12121212-3434-4565-8787-$(printf '%012d' $$)"
...
A_ID="$(qa_actor_account alice "a27-$N@archicode.codes" "pw-a-$N")"; A_TOK="$(cat "$QA_RESULTS/actors/alice/session.tok")"
B_ID="$(qa_actor_account bob "b27-$N@archicode.codes" "pw-b-$N")"; B_TOK="$(cat "$QA_RESULTS/actors/bob/session.tok")"
C_ID="$(qa_actor_account carol "c27-$N@archicode.codes" "pw-c-$N")"; C_TOK="$(cat "$QA_RESULTS/actors/carol/session.tok")"
...
for pair in "bob:b27" "carol:c27"; do
	...
	tok="$(cat "$QA_RESULTS/actors/$who/session.tok")"
...
assert_green "an org member with no grant at all reads neither environment" \
	-- bash -c 'qa_actor_reset dave >/dev/null
		...
		qa_api POST /v1/orgs/invites/accept "$(cat "$QA_RESULTS/actors/dave/session.tok")" "{\"token\":\"$t\"}" >/dev/null
