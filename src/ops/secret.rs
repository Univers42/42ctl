...
//! `set` and `get` (plus the shared `current_version`/`fetch_plaintext` helpers). `set`
//! seals the plaintext locally for the caller and pushes the opaque envelope with
//! optimistic concurrency (expected_prev = current head). `get` fetches the envelope and
//! decrypts it locally — the server sees neither plaintext nor a DEK at any point.
...
impl Session {
    ...
    pub(crate) async fn current_version(&mut self, path: &str) -> anyhow::Result<u64> {
        let mut request = Request::new(GetRequest {
            path: path.to_string(),
            version: 0,
        });
