...
//! expected id locally to bind a read to the right secret (anti-substitution). The
//! namespace is byte-identical to vault42's so 42ctl interoperates with the same server.

use uuid::Uuid;

/// Fixed namespace for vault42 secret ids (must match the server's namespace).
