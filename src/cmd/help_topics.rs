...
## Command groups
    identity & access    auth   keys   config
...
$ 42ctl config show                                       # the default profile is the public duo
$ 42ctl config endpoint --server https://vault42.fly.dev --authority https://grobase-nano.fly.dev --grobase https://grobase-stack.fly.dev
...
## Endpoints
$ 42ctl config endpoint --server <vault42 URL> --authority <authority URL> --grobase <grobase URL>
  server      vault42-server — the gRPC store that holds ciphertext
  authority   the contract authority — issues your login contract
  grobase     grobase — email OTP, keystore escrow, RBAC
